...
def main():
    ...
    print("Entrou aqui!")
    ref_f = gera_valores(alpha, 10000, 1000000)
...
def gera_valores(theta, k, N): 
    ...
    for c in range(0, 200):                                             # Aquecimento da cadeia
        ...
        if alpha >= 1:                                                 # Aceitar ou rejeitar de acordo com o critério do algorítmo
            f[c] = theta_2
        else:
...
def calcula_U_teste(f, k, N, v, ref_f):
    ref = calcula_U(ref_f, 10000, 100000, v, 0)
    print(ref)
    arr = np.zeros(100)
    tempo = np.zeros(100)
